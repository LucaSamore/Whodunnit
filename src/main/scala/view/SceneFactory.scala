...
/** Factory for creating scenes with dependency injection.
  * 
  * This factory uses the Cake Pattern to automatically inject dependencies into scenes,
  * ensuring that each scene has access to the required controllers and navigation capabilities.
  */
trait SceneFactory[S]:
  /** Creates a scene based on the requested page type */
  def createScene(page: ScenePage): Scene
  
  /** Navigates to a specific page by creating and displaying the corresponding scene */
  def navigateTo(page: ScenePage): Unit =
    WhodunnitApp.changeScene(createScene(page))

object SceneFactory:
  
  /** Factory implementation that requires dependencies through the Cake Pattern.
    * 
    * This component provides concrete scene implementations with proper dependency injection,
    * ensuring each scene can access the controller and navigation functionality.
    */
  trait Component[S]:
    ...
    class SceneFactoryImpl extends SceneFactory[S]:
      ...
      private class HomepageSceneImpl extends HomepageScene[S]:
        override protected def controller = context.controller
        override protected def navigateTo(page: ScenePage) = 
          SceneFactoryImpl.this.navigateTo(page)

      private class GameConfigurationSceneImpl extends GameConfigurationScene[S]:
        override protected def controller = context.controller
        override protected def navigateTo(page: ScenePage) = 
          SceneFactoryImpl.this.navigateTo(page)
