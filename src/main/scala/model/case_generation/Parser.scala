...
object JsonParser extends Parser:
  ...
  private def extractFieldName(errorMsg: String): String =
    val fieldPatterns = Map(
      ...
      "firstEntity" -> List("firstEntity"),
      "secondEntity" -> List("secondEntity"),
      "semantic" -> List("semantic"),
      "motive" -> List("motive")
    )

    fieldPatterns.find((_, patterns) => patterns.exists(errorMsg.contains))
      .map(_._1)
      .getOrElse("unknown")
  ...
  private case class CaseModel(
    ...
    def toCase: Either[ParseError, Case] =
      for
        plotObj <- plot.toPlot
        characterSet <- characters.traverse(
          _.toCharacter
        ).flatMap(_.toSet.validateNonEmpty("characters"))
        caseFiles <- files.traverse(
          _.toCaseFile(characterSet)
        ).flatMap(_.toSet.validateNonEmpty("files"))
  ...
  private case class PlotModel(title: String, content: String):
    def toPlot: Either[ParseError, Plot] =
      if title.isEmpty then
        Left(InvalidFieldError("plot.title", "must not be empty"))
      else if content.isEmpty then
        Left(InvalidFieldError("plot.content", "must not be empty"))
  ...
  private case class CharacterModel(name: String, role: String):
    def toCharacter: Either[ParseError, Character] =
      for
        _ <- name.nonEmpty.toRight(InvalidFieldError(
          "character.name",
          "must not be empty"
        ))
        roleValue <- parseRole(role).toRight(InvalidFieldError(
          "character.role",
          s"Unknown role: $role"
        ))
  ...
  private case class CaseFileModel(
    ...
    def toCaseFile(characters: Set[Character]): Either[ParseError, CaseFile] =
      for
        _ <- title.nonEmpty.toRight(InvalidFieldError(
          "file.title",
          "must not be empty"
        ))
        _ <- content.nonEmpty.toRight(InvalidFieldError(
          "file.content",
          "must not be empty"
        ))
        fileType <- parseFileType(kind).toRight(InvalidFieldError(
          "file.kind",
          s"Unknown type: $kind"
        ))
    ...
    def toSolution(
      ...
      for
        culpritChar <- characters.find(_.name == culprit)
          .toRight(InvalidFieldError(
            "solution.culprit",
            s"Character '$culprit' not found"
          ))
        prerequisites <-
          prerequisite.traverse(_.toKGPrerequisite(characters, files))
  ...
  extension [A](list: List[A])
    ...
    private def traverse[B](f: A => Either[ParseError, B])
        : Either[ParseError, List[B]] =
  ...
  private def parseTimestampEither(tsStr: String)
      : Either[ParseError, java.time.LocalDateTime] =
